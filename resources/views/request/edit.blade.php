        ...
        <div class="row justify-content-center">
            <div class="container">
                <div class="col-md-8 offset-md-2 col-sm-6">
                    <div class="card">
                        <div class="card-header">{{ __('Lista de Pedido') }}</div>
                        <div class="card-body">
                            @include('_include._flash-message')

                            <p>Numero do Pedido: # {{$request}}</p>
                                            ...
                                            @foreach ($items as $item)
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="text" name="lists[{{$item->id}}][box_qtd]" value="{{ $item->box_qtd }}"
                                                    ...
                                                    <td>
                                                        <select name="lists[{{$item->id}}][status]" class="form-control">
                                                            ...
                                                            <option value="0" {{$item->status_product == 0 ?'selected':''}} >Verde</option>
                                                            <option value="1" {{$item->status_product == 1 ?'selected':''}} >Amarelo</option>
                                                            <option value="2" {{$item->status_product == 2 ?'selected':''}} >Vermelho</option>
                                                        </select>
                                                    </td>
                                                </tr>
                                            @endforeach
